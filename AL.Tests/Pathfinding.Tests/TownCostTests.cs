...
/// <summary>
///     What a town channel is priced at, against the arithmetic written out here rather than read from the constants the
///     code uses.
/// </summary>
/// <remarks>
///     The rule being restated: the channel is a fixed three seconds and every other edge is a distance, so what the
///     channel is worth is the ground this character would have covered on foot in those three seconds - times a premium,
///     because a walk of the same length has made progress by the time it ends and an interrupted channel has not. Needs
///     no mesh, which is the whole reason it is a test.
/// </remarks>
public class TownCostTests
{
    /// <summary>
    ///     A slow character reaches for the channel more readily, because walking buys it less.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     Twice the speed covers twice the ground, so the channel it replaces is worth twice as much.
    /// </summary>
    [Test]
