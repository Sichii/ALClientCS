...
public class FindPathTests : PathfindingTestBed
{
    ...
    /// <summary>
    ///     Whether the straight line to the destination is blocked or clear, every leg is one the server accepts and
    ///     ends somewhere the flood fill reached. A leg walked through what lies between the two reads at runtime
    ///     exactly like a path the search failed to find: a walk that goes nowhere.
    /// </summary>
    [Test]
    public async Task NoLegCrossesAWallWhetherTheLineIsBlockedOrClear()
    {
        ...
        blocked.Should()
               .NotContain(
                   edge => Pathfinder.IsWalkable(edge.End) == false,
    ...
    /// <summary>
    ///     Two ends landing in one triangle is an ordinary set rather than a caller's mistake: any two destinations
    ///     closer together than the triangulation is fine share a triangle, and a monster with overlapping spawn
    ///     areas hands over exactly that - fireroamer's two on desertland are the pair that found this.
    /// </summary>
    /// <remarks>
    ///     The lookup from the mesh back to the destination used to be built straight off a key selector, so the
    ///     second end sharing one threw a duplicate key. Callers cannot tell that throw from the one the search makes
    ///     for a destination nothing walks to, so a monster standing on open ground reported as unreachable.
    /// </remarks>
    [Test]
    ...
    /// <summary>
    ///     What the funnel owes its caller: every walk leg is one the server accepts, consecutive walks hand off
    ///     exactly, and no pair of them is left that a straight line could still have collapsed. No leg count is
    ///     pinned - that would only fix whatever the pull happens to do today.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     The corner half of the same debt: a bend the pull kept is a bend worth walking to. Restated independently
    ///     of the pull by stepping along every leg of a walk run - a cut from there straight to any later corner of
    ///     the run that the server would allow may not save more than a step over walking the corners.
    /// </summary>
    [Test]
            ...
            for (var i = 0; (i + 1) < path.Length; i++)
            {
                ...
                for (var offset = 5f; offset < legLength; offset += 5f)
                {
                    ...
                    for (var j = runEnd; j > i; j--)
                    {
                        ...
                        for (var k = i + 1; k <= j; k++)
                            currentCost += path[k].Start.Distance(path[k].End);
    ...
    /// <summary>
    ///     Two searches at once do not share state: scratch is per thread, and the graph is never written after
    ///     Initialize. Each search checks its own answer against the same search run alone.
    /// </summary>
    [Test]
    public async Task SearchesRunConcurrently()
    {
        var start = new Location("main", -1582, 496);
        Destination[] ends = [new(new Location("halloween", 0, 0), 0), new(new Location("winter_cave", -84, 0), 0)];

        var alone = ends.Select(end => Pathfinder.FindPath(start, [end]).Sum(edge => edge.Cost)).ToArray();

        var together = await Task.WhenAll(
            Enumerable.Range(0, 16)
                      .Select(i => Task.Run(() => Pathfinder.FindPath(start, [ends[i % 2]]).Sum(edge => edge.Cost))));
    ...
    /// <summary>
    ///     A start that already satisfies the end still yields one leg, so a caller that takes the last leg as its
    ///     arrival finds where it stands there, and the leg walks nowhere: exactly on the end, and inside its radius.
    /// </summary>
    [Test]
    [Arguments(0f, 0f)]
    [Arguments(10f, 0f)]
    ...
    /// <summary>
    ///     Recall off means no recall anywhere on the route rather than none out of the start. The recall grafted
    ///     onto every arrival node is the same move as the one out of the start, and a caller that cannot recall
    ///     here cannot recall two maps along either - so a route that leans on one mid-way has to be found without.
    /// </summary>
    [Test]
    public async Task RecallOffSuppressesEveryRecallOnTheRoute()
    {
        ...
        withRecall.Where((edge, i) => (i > 0) && (edge.Type == EdgeType.Town) && (withRecall[i - 1].Type != EdgeType.Walk))
                  .Should()
                  .NotBeEmpty(
                      $"the premise is a recall taken mid-route: {string.Join(" ", withRecall.Select(edge => edge.Type))}");
