...
/// <summary>
///     A mesh built by hand: the unit square from (0, 0) to (100, 100) split along its diagonal into T0 below the diagonal
///     (v0, v1, v2) and T1 above it (v0, v2, v3). Neighbour slot i is across the edge opposite corner i.
/// </summary>
public class TriangleMeshTests
{
    /// <summary>
    ///     Two triangles touching only at v2 (50, 50): T0 (v0, v1, v2) below it and T1 (v2, v4, v3) above it, with no shared
    ///     edge, so the fan round v2 is two sectors.
    /// </summary>
    ...
    public void EveryVertexKnowsATriangleItBelongsTo()
    {
        ...
        for (var vertex = 0; vertex < mesh.Vertices.Length; vertex++)
        {
            ...
            ((mesh.Corners[triangle * 3] == vertex)
             ...
             || (mesh.Corners[triangle * 3 + 2] == vertex)).Should()
                                                           .BeTrue();
        }
    }
