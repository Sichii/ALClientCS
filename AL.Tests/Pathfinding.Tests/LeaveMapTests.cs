...
/// <summary>
///     Holds the ways off an irregular map to what the server actually accepts, restated here from its own source
///     rather than read from the graph the tests are checking.
/// </summary>
/// <remarks>
///     Two rules, and the code used to have neither. The leave command is taken from jail and cyberland only - the
///     server checks the map by name, plus solo instances, which only a gm can create - and it hands the character to
///     the start map's first spawn whatever doors the map also has. A door is a separate way out, walked to and used
///     with transport, and being irregular has nothing to do with whether a map has one.
/// </remarks>
public class LeaveMapTests : PathfindingTestBed
{
    ...
    /// <summary>
    ///     The character arrives on the town spawn already, so a recall there moves it nowhere and costs it three
    ///     seconds of channel it can be knocked out of.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     With recall off the table entirely there has to still be a path, which is what fails when the arrival node
    ///     has no walk out of it. A recall that gets interrupted is retried exactly this way.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     Cyberland keeps both ways out, and the door is the shorter one when the trip ends near where it lands - a
    ///     leave costs the same from anywhere but arrives 400 units further from the transporter.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     Resort is irregular and the server refuses a leave from it, so its door to the tavern is the only way out
    ///     it has. Skipping the doors over irregular maps left it with none at all.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     The server's list, written out from its handler rather than read from the code under test. Being irregular
    ///     is not what decides it - duelland and resort are irregular and refuse the command.
    /// </summary>
    [Test]
    [Arguments("jail", true)]
