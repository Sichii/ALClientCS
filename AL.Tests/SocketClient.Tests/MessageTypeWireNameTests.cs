...
///     <see cref="ALSocketMessageType" /> via <c>EnumHelper.TryParse</c> (OrdinalIgnoreCase over the member name and its
///     [EnumMember] value). This pins every tier-1 inbound member to the exact server wire name, so a rename or a missing
///     [EnumMember] fails the build's tests.
/// </summary>
