...
{
    /// <summary>
    ///     server.js:4562 - a cross-server whisper reaches the recipient with an extra 'xserver' key and the sender's owner id
    ///     in place of a character id. Nothing maps either, and a throw here would escape into ALSocketClient.OnAny and
    ///     discard the frame.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     The same event carries NPC and monster chatter through xy_emit, which is local rather than server-wide and which
    ///     nobody can answer. server_functions.js:2252 is the Grinch, and it is the one that carries a colour.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     server.js:4601-4602 emits a whisper to both ends, and 'to' is populated only on the sender's copy - so it is the
    ///     only thing that identifies a message this character sent rather than received.
    /// </summary>
    [Test]
