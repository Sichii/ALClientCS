...
{
    /// <summary>
    ///     Boxes are centred horizontally but rise from the entity's feet, so the whole height sits on one side of the centre
    ///     point and nothing on the other. Getting that backwards halves the vertical reach and leaves the horizontal one
    ///     untouched, which is the sort of error only a fixed number catches.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     A monster with no entry in the dimensions table is squared off at 24 rather than left without a box, and the
    ///     handful carrying a size multiplier are scaled and rounded before anything measures them. A crab is half size, so
    ///     this is worth several units on the most common target in the game.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     A monster is sized the same way from its own entry, and from the raw width and height rather than the scaled-down
    ///     pair the nav mesh is padded with - those are two different boxes for two different jobs, and only this one decides
    ///     what is in range.
    /// </summary>
    [Test]
