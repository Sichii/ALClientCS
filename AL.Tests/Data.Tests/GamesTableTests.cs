...
public class GamesTableTests
{
    ...
    /// <summary>
    ///     A slice's three slots are its own name, the side it pays and a display colour. The first and third are both
    ///     colour words, so a swap binds without complaint and silently makes every slice pay the same side.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     A prize's three slots are the item, what it pays in gold, and how many draws land on it. Payout and weight are
    ///     both bare numbers, so a swap binds cleanly and inverts the table - the rarest outcome becomes the most common.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     Hand rankings and card ranks are both compared by position rather than by any value in the entry, so the order
    ///     they arrive in is the whole of the rule.
    /// </summary>
    [Test]
