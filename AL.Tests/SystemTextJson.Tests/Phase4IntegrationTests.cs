...
///     <see cref="ALJson.Options" />/<see cref="SocketJson.Options" /> (the socket + G-data paths that Newtonsoft
///     used to serve): tolerant enums as dictionary keys, positional types whose constructor carries an unbound
///     parameter, and the read-only <see cref="Inventory" /> collection.
/// </summary>
[TestClass]
public sealed class Phase4IntegrationTests
{
    ...
    public void TolerantEnum_BindsAsDictionaryKey()
    {
        ...
        byWeapon.Should().ContainKey(WeaponType.Spear).WhoseValue.Should().Be(5);
        byWeapon.Should().ContainKey(WeaponType.Bow).WhoseValue.Should().Be(7);
    }

    [TestMethod]
    public void TolerantEnum_UnknownDictionaryKey_DegradesToDefault()
    {
        // an unrecognized key degrades to the enum's zero member instead of throwing (Newtonsoft threw here - a
        // conscious Phase-5 re-baseline). The default key does not collide with a real one in this payload.
        var parsed = JsonSerializer.Deserialize<Dictionary<WeaponType, int>>("""{"totally_bogus":9}""", ALJson.Options)!;

        parsed.Should().ContainKey(default(WeaponType)).WhoseValue.Should().Be(9);
    }
    ...
    public void StraightLine_BindsFromPositionalArray_DefaultingUnboundCtorParameter()
    {
        ...
        line.On.Should().Be(10);
        line.Start.Should().Be(20);
        line.End.Should().Be(30);
    ...
    public void Inventory_BindsFromItemArray()
    {
        ...
        inventory[1].Should().BeNull();
        inventory[2]!.Name.Should().Be("mpot0");
    }
}
