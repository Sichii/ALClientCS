...
/// <summary>
///     STJ Phase 1 self-check for the ported leaf converters. Not the full equivalence gate (that is Phase 5, against the
///     committed snapshot fixtures) — this pins the highest-risk behaviours now: the tolerant enum factory + nullable-enum
///     path, degrade-to-zero, lowercase emit, tuple truncation, and falsy/array handling, so a port regression fails here
///     rather than four phases later.
/// </summary>
public sealed class LeafConverterTests
{
    ...
    public void ArrayOrSingle_Null_Single_Array()
    {
        ...
        JsonSerializer.Deserialize<IReadOnlyList<TColor>>("null", options)
                      .Should()
                      .BeEmpty();

        JsonSerializer.Deserialize<IReadOnlyList<TColor>>("\"red\"", options)
                      .Should()
                      .Equal(TColor.Red);

        JsonSerializer.Deserialize<IReadOnlyList<TColor>>("""["red","grn"]""", options)
                      .Should()
                      .Equal(TColor.Red, TColor.Green);
    }
    ...
    public void FalsyList_False_EmptyList_Array_Parsed()
    {
        ...
        JsonSerializer.Deserialize<IReadOnlyList<int>>("false", options)
                      .Should()
                      .BeEmpty();

        JsonSerializer.Deserialize<IReadOnlyList<int>>("[1,2]", options)
                      .Should()
                      .Equal(1, 2);
    }
    ...
    {
        JsonSerializer.Deserialize<TSlot>("\"mainhand\"", Opts())
                      .Should()
                      .Be(TSlot.MainHand);

        JsonSerializer.Serialize(TSlot.MainHand, Opts())
                      .Should()
                      .Be("\"mainhand\"");
    }
    ...
    {
        JsonSerializer.Deserialize<TColor>("\"grn\"", Opts())
                      .Should()
                      .Be(TColor.Green);

        JsonSerializer.Deserialize<TColor>("\"RED\"", Opts())
                      .Should()
                      .Be(TColor.Red);
    }
    ...
    public void TolerantEnum_Nullable_Null_StaysNull()
        => JsonSerializer.Deserialize<Holder>("""{"c":null}""", Opts())!.Color
                         .Should()
                         .BeNull();

    [Test]
    public void TolerantEnum_Nullable_Unknown_DegradesToZero()
        => JsonSerializer.Deserialize<Holder>("""{"c":"nope"}""", Opts())!.Color
                         .Should()
                         .Be(TColor.None);
    ...
    public void TolerantEnum_Unknown_DegradesToZero()
        => JsonSerializer.Deserialize<TColor>("\"nope\"", Opts())
                         .Should()
                         .Be(TColor.None);
    ...
    {
        (var a, var b) = JsonSerializer.Deserialize<(string, int)>("""["x",5]""", Opts(new ArrayToTupleConverter<string, int>()));
    ...
    {
        (var a, var b) = JsonSerializer.Deserialize<(string, int)>("""["x",1,2]""", Opts(new ArrayToTupleConverter<string, int>()));

        a.Should()
         .BeNull("an over-length array deliberately falls to the converter's _ => default arm");
    ...
    {
        (var a, var b) = JsonSerializer.Deserialize<(string, int)>("""["only"]""", Opts(new ArrayToTupleConverter<string, int>()));
