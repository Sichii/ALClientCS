...
/// <summary>
///     STJ Phase 2 mechanics check for the recursion-safe attributed-object converter. Not the full equivalence
///     gate (Phase 5, against real GItem/entity frames once the models are STJ-annotated) — this validates the
///     converter LOGIC in isolation with purpose-built STJ-annotated types: recursion-safe declared fill, the
///     every-key presence pass, numeric-ALAttribute harvest (incl. property-backed), non-numeric keys not
///     throwing, GItem-style scroll-stat recovery, and lenient int rounding on the inner path.
/// </summary>
[TestClass]
public sealed class AttributedObjectConverterTests
{
    ...
    public void Entity_FillsProps_MarksPresence_HarvestsNumericAttributes()
    {
        ...
        entity.Attack.Should().Be(5);
        entity.Armor.Should().Be(3);

        // every top-level wire key marked present, including the non-numeric one
        entity.PresentKeys.Should().BeEquivalentTo("attack", "armor", "heal");

        // numeric ALAttribute keys harvested (property-backed: Attack is both a prop AND a dict entry)
        entity.Attributes.Should().ContainKey(ALAttribute.Attack).WhoseValue.Should().Be(5);
        entity.Attributes.Should().ContainKey(ALAttribute.Armor).WhoseValue.Should().Be(3);

        // a non-numeric ALAttribute-named key (heal=true) is skipped, never harvested, and does not throw
        entity.Attributes.Should().NotContainKey(ALAttribute.Heal);
    }
    ...
    public void GItem_RecoversScrollStat_KeepsStatZero_RoundsFractionalInt()
    {
        ...
        item.RecoveredScroll.Should().Be(ALAttribute.Int);

        // the rest of the object still binds (Newtonsoft's [OnError] resumed; the strip achieves the same)
        item.Attack.Should().Be(5);

        // lenient int rounding on the inner path: 3.6 -> 4 (banker's rounding), matching JToken.ToObject<int>
    ...
    public void NullToken_YieldsNull()
        => JsonSerializer.Deserialize<TestEntity>("null", Opts()).Should().BeNull();
    ...
    public void NestedAttributed_StillHarvests()
    {
        ...
        container.Attributes.Should().ContainKey(ALAttribute.Attack).WhoseValue.Should().Be(1);
        container.Child.Should().NotBeNull();
        container.Child!.Armor.Should().Be(9);
        container.Child.Attributes.Should().ContainKey(ALAttribute.Armor).WhoseValue.Should().Be(9, "the nested attributed value still harvests");
    }
