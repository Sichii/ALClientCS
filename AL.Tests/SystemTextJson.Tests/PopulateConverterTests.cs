...
/// <summary>
///     STJ Phase 2 mechanics check for the recursion-prone reusable converters (array-to-object, string-or-object, falsy).
///     Each is registered IN the options — the configuration that triggers self-recursion — so if
///     <see cref="RecursionSafeOptions" /> failed to drop the converter for the inner deserialize, these would
///     StackOverflow and crash the host (as an unguarded self-deserialize did in Phase 1) rather than fail an assertion.
///     EventAndBoss/Disappear share the pattern but register as concrete instances rather than factories, so they get
///     their own end-to-end pair at the bottom of this file.
/// </summary>
public sealed class PopulateConverterTests
{
    ...
    {
        var data = TestJson.Socket<DisappearData>("""{"id":"CeeNote","reason":"transport","to":"main","invis":true,"s":[5,10,1]}""")!;
        ...
        data.ToMap
            .Should()
            .Be("main", "the renamed declared member bound under the recursion-safe options");
        ...
        data.ToOrientation
            .Should()
            .NotBeNull("the 's' special case still runs after the inner fill");
    ...
    public void EventAndBoss_ViaSharedOptions_FillsFlagsAndBosses_NoStackOverflow()
    {
        ...
        data.Valentines
            .Should()
            .BeFalse("an absent flag keeps its default");

        data.BossInfo
            .ContainsKey("icegolem")
            .Should()
            .BeTrue("every remaining object-valued key is a boss");

        data.BossInfo["icegolem"]
            .Id
            .Should()
            .Be("icegolem", "the converter stamps each boss with its wire key");
    ...
    public void Falsy_Enum_False_Default_KnownParses_UnknownDefaults()
    {
        ...
        JsonSerializer.Deserialize<FEnum>("false", options)
                      .Should()
                      .Be(FEnum.None);

        JsonSerializer.Deserialize<FEnum>("\"GREEN\"", options)
                      .Should()
                      .Be(FEnum.Green);

        JsonSerializer.Deserialize<FEnum>("\"grn\"", options)
                      .Should()
                      .Be(FEnum.Green, "the EnumMember alias resolves");

        JsonSerializer.Deserialize<FEnum>("\"zzz\"", options)
                      .Should()
                      .Be(FEnum.None, "an unknown name degrades to the default");
    }
    ...
    public void Falsy_Int_FalseAndNull_YieldDefault_ValuePassesThrough()
    {
        ...
        JsonSerializer.Deserialize<int>("false", options)
                      .Should()
                      .Be(7);
        ...
        JsonSerializer.Deserialize<int>("5", options)
                      .Should()
                      .Be(5);
    }
    ...
    public void StringOrObject_String_SetsNamedProperty_ContainsDataFalse()
    {
        ...
        value.ContainsData
             .Should()
             .BeFalse("a bare string is the scalar shape");
    }

    [Test]
    public void StringOrObject_ViaFactory_Object_Populates_NoStackOverflow()
    {
        var value = JsonSerializer.Deserialize<TestOptional>("""{"name":"world","count":3}""", Opts(new StringOrObjectConverterFactory()))!;
