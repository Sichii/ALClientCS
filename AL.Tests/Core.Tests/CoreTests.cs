...
{
    /// <summary>
    ///     An entity's movement is written by three threads that never coordinated: the 30Hz reckoning loop, the socket's
    ///     receive callbacks, and the consumer's own movement calls. Each write is a read-compute-write, so a write landing
        ...
        var mover = Task.Run(() =>
        {
            ...
            try
            {
                for (var round = 0; round < ROUNDS; round++)
                {
                    ...
                    if (movement.X.IsNear(movement.GoingX, CONSTANTS.EPSILON) && movement.Y.IsNear(movement.GoingY, CONSTANTS.EPSILON))
                        continue;
    ...
    /// <summary>
    ///     Several paths write an entity's position between ticks without re-deriving <see cref="EntityBase.Angle" /> - the
    ///     server's correction handler, the in-leg repair, the character branch of an entities frame. A step that steered by
    ///     the stored heading would resume from the corrected position along the old line, walking parallel to the one it
    ///     should be on until the overshoot clamp finally tripped. Pinned here alongside the two edges the derived heading
    ///     still has to keep: a step short of the goal, and one that would pass it.
    /// </summary>
    [Test]
    ...
    ///     would never have walked, which reads as a desync rather than as a fix.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     The point-to-box reading follows the same rule: an axis the point already lies within is not part of the distance.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     Separated on both axes it is the diagonal of the two gaps, and any overlap at all reads as zero rather than going
    ///     negative.
    /// </summary>
    [Test]
    public void EdgeToEdgeDistanceIsDiagonalWhenApartOnBothAxes()
    {
        ...
        var lower = new BoundingRectangle(
            0,
            0,
            5,
            10,
        ...
        var upper = new BoundingRectangle(
            14,
            -13,
            5,
            10,
        ...
        var overlapping = new BoundingRectangle(
            3,
            0,
            5,
            10,
    ...
    /// <summary>
    ///     The server resolves every attack, skill and aggro check with the gap on each axis independently, each clamped at
    ///     zero. An axis the two boxes already overlap on contributes nothing, so two entities standing alongside each other
    ///     are exactly their horizontal gap apart however tall either of them is.
    /// </summary>
    [Test]
    public void EdgeToEdgeDistanceIsPerAxisSeparation()
    {
        ...
        var left = new BoundingRectangle(
            0,
            0,
            5,
            35,
        ...
        var right = new BoundingRectangle(
            20,
            0,
            5,
            20,
        ...
        var raised = new BoundingRectangle(
            20,
            -10,
            5,
            20,
    ...
    /// <summary>
    ///     The reading a melee character actually depends on. The boxes are bottom-anchored and rise from the entity's feet,
    ///     so alongside each other the half-widths are the whole of the difference: a character and a mole standing 43 apart
    ///     are 15 apart by the measure the server grants the mole its attack with.
    /// </summary>
    [Test]
    public void EdgeToEdgeDistanceMatchesTheServerForACharacterAndAMonster()
    {
        //a player is 26 wide and 36 tall; a mole is 30 by 20
        var character = new BoundingRectangle(
            0,
            0,
            13,
            36,
        ...
        var mole = new BoundingRectangle(
            43,
            0,
            15,
            20,
    ...
    public void RayTraceYieldsWholeNumbersFromFractionalEnds()
    {
        ...
        [
            new Point(0.4f, 0.6f),
            new Point(-7.25f, 3.75f),
            new Point(12.5f, -12.5f)
        ];

        IPoint[] ends =
        [
            new Point(9.9f, -4.1f),
            new Point(-7.25f, 20.5f),
            new Point(30.5f, -12.5f)
        ];
