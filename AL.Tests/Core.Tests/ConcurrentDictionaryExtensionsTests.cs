...
/// <summary>
///     LINQ ToList on a ConcurrentDictionary reads Count then CopyTo as two operations. A writer between them throws
///     ArgumentException - the crash in ALClient.Update when a skill_timeout lands during the cooldown tick. That is the
///     ICollection&lt;KeyValuePair&gt; path only; the projectile sweep went through Values, which copies under lock and
///     was never at risk. These pin the in-place walk that replaced both.
/// </summary>
public class ConcurrentDictionaryExtensionsTests
{
    ...
    /// <summary>
    ///     TryRemove(kvp) compares the value, so what it protects is TValue's equality and not the walk's. Both spellings are
    ///     pinned because the production callers are split across them - Cooldowns is a class, Conditions and Projectiles are
    ///     records - and the remark on the extension says so.
    /// </summary>
    [Test]
