...
public class GameDataLoadCharacterization
{
    ...
    public void ALookMissingItsPieceMapBindsNull(string wire, string because)
        => TestJson.Data<GClassLook>(wire)!.Pieces
                   .Should()
                   .BeNull(because);
    ...
    /// <summary>
    ///     The hazard the push covers, stated from the raw entry as well as from the loaded one: four of the seven classes
    ///     carry no exclusive list at all, so binding alone would leave them nothing to wear.
    /// </summary>
    [Test]
    public void T1_ClassCosmetics_AClassTheSnapshotCarriesNoListForStillEndsUpWithOne()
    {
        ...
        TestJson.Data<GClass>(raw.ToJsonString())!.ExclusiveCosmetics
                .Should()
                .NotBeNull()
                .And
                .BeEmpty();

        GameData.Classes["paladin"]!.ExclusiveCosmetics
                .Should()
                .NotBeNull()
                .And
                .NotBeEmpty();
    ...
    /// <summary>
    ///     The class half of the enrichment pass. The wire form carries the raw grant and nothing else, so a class is only
    ///     right once the load has restated the push the server does while it processes game data
    ...
    /// <summary>
    ///     The drop entries are the one positional shape whose third slot means two different things, so all three forms are
    ///     pinned here rather than only the count.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     The three halves of the drop data that used to be discarded. The prize tables are keyed by the game's own drop id,
    ///     which is an item name for most of them and something else entirely for the rest.
    /// </summary>
    [Test]
    ...
    ///     back that far would look harmless and leave 34 exchangeables with nowhere to go. These are the snapshot's counts,
    ///     and the errands' prose cites the snapshot too, attributing it, so this assertion is what holds those comments as
    ///     well as the rule. That is the whole reason to cite the fixture: it is the only number anything can check.
    /// </summary>
    [Test]
    public void T1_ExchangeAtNPC_CoversEveryExchangeable()
    {
        ...
        GameData.Items["leather"]!.ExchangeAtNPC!.Id
                .Should()
                .Be("leathermerchant");

        GameData.Items["gem0"]!.ExchangeAtNPC!.Id
                .Should()
                .Be("exchange");
    }

    /// <summary>
    ///     The exchange prizes, keyed the way the server keys them. The lost earring is the whole reason the key is a level
    ///     rather than a name: its five tables are five different prizes, not five rates on one.
    /// </summary>
    [Test]
    public void T1_ExchangeRewards_AreKeyedByTheLevelExchanged()
    {
        ...
        GameData.Items["gem0"]!.ExchangeRewards
                .Should()
                .ContainKey(0)
                .And
                .HaveCount(1);

        GameData.Items["lostearring"]!.ExchangeRewards
                .Should()
                .ContainKeys(
                    0,
                    1,
        ...
        GameData.Items["hpot0"]!.ExchangeRewards
                .Should()
                .BeNull();
    }
    ...
    {
        GameData.Maps["main"]!.Drops
                .Should()
                .Contain(drop => drop.Name == "ringsj");

        //empty rather than null, so a caller never has to ask which kind of nothing it got
        GameData.Maps["desertland"]!.Drops
                .Should()
                .BeEmpty();
    }
    ...
    /// <summary>
    ///     Pins the regeneration signal: the snapshot carries one member the generated datums do not declare, and an injected
    ///     unknown member is counted on top of it. The baseline alone could also mean the scan matched no sections at all,
