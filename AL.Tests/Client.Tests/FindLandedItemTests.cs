...
/// <summary>
///     FindLandedItem is how UnequipAsync decides which inventory slot the unequipped item landed in. The server rule
///     restated here rather than probed: add_item puts a non-stackable in the first free slot, while an equip in flight on
///     the same socket swaps in place - so of two identical items appearing at once, the unequipped one is the one on a
///     slot that was empty before the emit. A name-and-level search alone cannot tell them apart, and equipping the wrong
///     one's slot back is how a warrior cleaving with matched dual weapons ended up trying to wear its own axe as an
///     offhand.
/// </summary>
public class FindLandedItemTests
{
    ...
    public void ADifferentLevelIsNotTheItem()
    {
        ...
        inventory.FindLandedItem(new HashSet<int>(), "fireblade", 5)
                 .Should()
                 .BeNull();
    }
