...
///     equality remarks), which on a stack of anniversary gifts was the consumed input — the stats feed logged "got
///     anniversarygift × 7" then 6, 5, 4 as each exchange ate one. Restate the server's add_item / consume rule here: the
///     prize is a newly occupied slot, or a pile whose quantity rose after subtracting the units taken from the exchanged
///     slot.
/// </summary>
public class FindExchangePrizeTests
{
    ...
    public void ANewSlotIsThePrize()
    {
        ...
        var prize = after.FindExchangePrize(before, 0, 1);
    ...
    public void ConsumptionAloneIsNotAPrize()
    {
        ...
        after.FindExchangePrize(before, 0, 1)
             .Should()
             .BeNull();
    }
