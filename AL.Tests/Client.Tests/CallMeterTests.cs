...
/// <summary>
///     The client's half of the server's rate limiter: what an emit costs, and how the window apportions it. The costs
///     themselves are the server's own table (node/server.js:144), so a wrong figure here is a bot that runs confidently
///     into a disconnect it could have seen coming.
/// </summary>
public class CallMeterTests
{
    /// <summary>
    ///     The transport handler bills the bank mount as 32 and the unmount as 16 under their own name, beside the 8 every
    ///     transport pays (node/server.js:5569, :5580). Between two bank floors it is a door like any other.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     A chest opened in a party resends every member at the open_chest modifier and bills the opener for all of it: a
    ///     tenth for a member who got nothing, four tenths for one who got an item, nothing for the opener's own item
    ///     (node/server.js:10460).
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     The server's add_call_cost folds a charge into its last entry when the method matches and keeps that entry's date,
    ///     so a run of one method expires as a block dated from its first call. Restated here rather than read off the meter,
    ///     so a meter that slid every entry on its own would go red.
    /// </summary>
    [Test]
    ...
    public void TheSnapshotSplitsTheSameWindowTwoWays()
    {
        ...
        var snapshot = meter.Snapshot(42d);
