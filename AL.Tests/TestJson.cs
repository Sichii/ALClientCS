...
///     DTO-contract tests exercise the same facade production uses. Replaces the <c>DeserializeObject&lt;T&gt;(json)</c>
...
internal static class TestJson
{
    ...
    /// <summary>
    ///     Outbound wire form, through the options the socket transport actually emits with ( <c>ALSocketClient</c> installs
    ///     <see cref="SocketJson.Options" />).
    /// </summary>
    /// <remarks>
    ///     Always go through this rather than calling <c>JsonSerializer.Serialize</c> inline: with default options
    ///     <c>TradeSlot.Trade1</c> emits as <c>17</c> rather than <c>"trade1"</c> , and the server acts on it — the exact slip
    ///     that once destroyed real inventory. One forgotten options argument is all it takes.
    /// </remarks>
