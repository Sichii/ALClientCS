...
///     the accrued cost of the last <see cref="WINDOW" /> passing <see cref="LIMIT" /> is a <c>limitdcreport</c> and
///     an immediate kick.
/// </summary>
/// <remarks>
///     A call is billed in two places, and <see cref="Of" /> is the sum of both: the method's row in the server's
///     <c>CC</c> table (node/server.js:159), which most methods have none of, and every <c>resend</c> the handler
///     runs before it returns (node/server.js:4017). A resend bills one unit when its events carry <c>u</c> and one
///     more when they lack <c>nc</c>, each at the method's <c>call_modifier</c>: 1 for everything but <c>skill</c>
///     (0.05), <c>target</c> (0.5) and <c>open_chest</c> (0.1). So an attack, <c>u+cid</c> at 1, is two; the same
///     resend from a skill is a fifth of one.
///     <br />
///     There is no per-call base charge. The wrapper's <c>add_call_cost(-1)</c> runs before <c>current_socket</c> is
///     set, and the previous handler left that pointing at the module's <c>false_socket</c> (node/server.js:4421),
///     so the unit never lands on a player. Billing one anyway reads a skill-heavy character at about double the
///     server; leaving the resends out reads an attack-heavy one at about half.
/// </remarks>
public static class CallCost
{
    ...
    /// <summary>What a <c>u+cid</c> resend bills at modifier 1: a unit for <c>u</c> and a unit for the stats pass.</summary>
    private const double RESEND = 2d;

    /// <summary><c>CC.equip</c>, kept apart from the equip row because <see cref="OfEquipBatch" /> scales it alone.</summary>
    private const double EQUIP_ROW = 3d;

    /// <summary>The <c>open_chest</c> modifier: what one unit of a resend costs from inside that handler.</summary>
    private const double OPEN_CHEST = 0.1d;

    /// <summary>What a resend that reopens somebody else's socket bills on top, in units (node/server.js:4051).</summary>
    private const double REOPEN_OTHER = 4d;

    /// <summary>
    ///     What <c>transport_player_to</c> bills whoever it moves (node/server.js:4180), from a loop as readily as
    ///     from a handler. Public for the one cast that ends in it - a landed blink is a skill at 0.1 and then this
    ///     when its condition runs out (node/server.js:13386), which no row keyed on the emit type can carry.
    /// </summary>
    ...
    /// <summary>
    ///     What one emit of this type costs against <see cref="LIMIT" />: its <c>CC</c> row plus what its handler's
    ///     resend bills, and nothing for a method that has neither.
    /// </summary>
    /// <remarks>
    ///     <see cref="ALSocketEmitType.EquipBatch" /> is priced here as a batch of one, because the count is not a
    ///     function of the type - ask <see cref="OfEquipBatch" /> when it is known.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    ///     <c>CC.equip * (0.5 + count/2)</c> (node/server.js:4357) plus the one <c>reopen+u+cid</c> resend the
    ...
    ///     (node/server.js:10460). A member who got nothing is an empty resend, a tenth; one who got an item is
    ///     reopened instead, which is free for the opener and <see cref="REOPEN_OTHER" /> tenths for anybody else.
    /// </summary>
    /// <param name="partySize">Everybody in the party, wherever they are - the server does not check the map.</param>
    /// <param name="othersWithItems">Members other than the opener who received an item from the chest.</param>
