...
/// <summary>
///     A map's wall lines in the shape the server keeps them, with the server's own segment test over them. This is
///     what the official client clips a move against, so it is what a planned leg has to pass.
/// </summary>
public sealed class WallLines
{
    ...
    private static (int[] On, int[] Start, int[] End) Sorted(IReadOnlyList<StraightLine> lines)
    {
        ...
        Array.Sort(order, (a, b) => lines[a].On.CompareTo(lines[b].On));
    ...
    /// <summary>
    ///     Whether a single track from (x0, y0) to (x1, y1) crosses no line. The server's single-track test, clause
    ///     for clause.
    /// </summary>
    ...
    /// <summary>
    ///     Whether a character with the given collision base can move from (x0, y0) to (x1, y1): the four corners of
    ///     the base, plus two fence tracks along the box's leading edges at the destination.
    /// </summary>
    public bool CanMove(
        double x0,
        double y0,
        ...
        BoundingBase boundingBase)
    {
        ...
        if (!CanMoveLine(x0 - h, y0 + vn, x1 - h, y1 + vn))
            return false;

        if (!CanMoveLine(x0 + h, y0 + vn, x1 + h, y1 + vn))
            return false;

        if (!CanMoveLine(x0 - h, y0 - v, x1 - h, y1 - v))
            return false;
        ...
        if (!CanMoveLine(x1 + px1, y1 + py0, x1 + px1, y1 + py1))
            return false;

        return CanMoveLine(x1 + px0, y1 + py1, x1 + px1, y1 + py1);
    }

    /// <summary>
    ///     Whether any line passes through the collision box hanging on (x, y). A point where this is true is one
    ///     the client would refuse every move out of, which is what a wall means at runtime.
    /// </summary>
