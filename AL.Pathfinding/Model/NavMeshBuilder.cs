...
public sealed class NavMeshBuilder
{
    ...
    private PolygonSet TracePolygons(ICollection<Point> vertices)
    {
        ...
        while (vertices.Count > 0)
        {
            ...
            var polyLine = FloodPolyLine(vertices)
                           .Select((point, i) => new PolygonPoint(point.X, point.Y, i))
                           .ToArray();
        ...
        {
            var holesOfThesePolygons = polygons.Where(p => (p != polygon) && p.ContainsPoint(polygon.Points[0]))
                                               .ToList();
        ...
        var allHoles = polygons.Where(p => p.Holes?.Count > 0)
                               .SelectMany(p => p.Holes);
