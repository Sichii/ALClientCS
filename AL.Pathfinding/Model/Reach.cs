...
/// <summary>
///     Where a walk may stop to count as having arrived: a rectangle band inflated by a range. The set of positions
///     that opens a door is exactly this shape (the server measures a box against a box, per axis, clamped at
///     zero), and a plain destination is a zero-size band with its radius as the range.
/// </summary>
public readonly struct Reach
{
    ...
    public static Reach Circle(float x, float y, float radius) => new(new Rectangle(x, y, 0f, 0f), radius);

    /// <summary>The distance from (x, y) to the band's edge, zero inside it.</summary>
    ...
    /// <summary>
    ///     The point on the reach's boundary nearest to (x, y): the clamp of the point into the band, stepped back
    ///     toward the point by the range. A point already inside answers itself.
    /// </summary>
    ...
    /// <summary>
    ///     The first point along the segment from (x0, y0) to (x1, y1) that lies inside the reach. The distance to a
    ///     convex region is convex along a line, so with the start outside and the end inside there is exactly one
    ///     crossing, found by bisection.
    /// </summary>
