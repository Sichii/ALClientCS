...
/// <summary>
///     Normalizes the login endpoint's two shapes into a <see cref="LoginResponse" />: success is a bare array
///     of notification objects; failure is an object carrying <c>failed</c>/<c>reason</c> that may wrap that
///     array under <c>infs</c>. The System.Text.Json replacement for the Newtonsoft
///     <c>LoginResponseConverter</c>.
/// </summary>
public sealed class LoginResponseConverter : JsonConverter<LoginResponse>
{
    public override LoginResponse? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {
            Failed = envelope?["failed"]?.GetValue<bool>() ?? false,
            Reason = envelope?["reason"]?.GetValue<string>(),
    ...
    private static string? FirstValue(IEnumerable<JsonObject> notifications, string propertyName)
        => notifications.Select(notification => notification[propertyName]?.GetValue<string>())
