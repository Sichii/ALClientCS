...
public sealed record GMonsterAbility : AttributedRecordBase
{
    ...
    ///     it comes off cooldown, with no attack roll (node/server.js:12655).
    /// </summary>
    ...
    /// <summary>
    ///     The cooldown of this ability in milliseconds. A monster spawns with a random fraction of it already elapsed, so a
    ///     pack does not fire in unison (node/server.js:12069).
    /// </summary>
    ...
    ///     <br />
    ///     This will not show up as the <see cref="Condition" /> for this ability. The server does not read the flag either -
    ///     it hard-codes the one ability that carries it, "putrid", which curses and poisons whoever hits the monster
    ///     (node/server.js:3651).
    /// </summary>
    ...
    ///     <br />
    ///     This will not show up as the <see cref="Condition" /> for this ability. See <see cref="Curse" /> - the same
    ///     "putrid" branch applies both, and the flag itself is never read.
    /// </summary>
    public bool Poison { get; init; }

    /// <summary>
    ///     Whether or not this ability does pure damage. Descriptive only: nothing reads it, and the skill the ability fires
    ///     already carries its own damage type.
    /// </summary>
    ...
    ///     not a centre-to-centre one (js/old_common_functions.js:618).
    /// </summary>
    public float Radius { get; init; }

    /// <summary>
    ///     On a burn ability, drops the divider the burn stack builds against from 3 to 1.5, so this monster's
    ///     <see cref="AL.Core.Definitions.Condition.Burned" /> gains intensity about twice as fast and forgoes the duration
    ///     extension the ordinary divider gets (node/server.js:3636).
    ///     <br />
