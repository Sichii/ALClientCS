...
public sealed record GMonster : AttributedRecordBase
{
    ...
    ///     monster tracker is equipped (node/server.js:1331).
    /// </summary>
    ...
    ///     every 1.2 seconds, and anything at or above 1 always passes (node/server.js:12791).
    /// </summary>
    ...
    /// <summary>
    ///     The speed this monster moves at the moment it has a target - any target, not just a <see cref="Rage" /> lock. It
    ///     replaces <see cref="AttributedRecordBase.Speed" /> outright rather than adding to it, and is often several times
    ///     larger. A <see cref="Supporter" /> following another monster uses it too, capped at that monster's speed plus 4
    ///     (node/server.js:1629).
    /// </summary>
    ...
    ///     whoever it was targeting (node/server.js:2584).
    /// </summary>
    ...
    ///     player it is fighting. Killing a player still levels it (node/server.js:14413, :12407).
    /// </summary>
    ...
    ///     drops no gold at all (node/server.js:2136). Most monsters do not carry one.
    /// </summary>
    ...
    ///     stands within 300 units, which dampens it instead (node/server.js:3237).
    /// </summary>
    ...
    ///     monster's (node/server.js:2142). Only the Grinch carries it.
    /// </summary>
    ...
    /// <summary>
    ///     If true, only basic attacks and the handful of skills flagged pierces_immunity damage this monster; everything else
    ///     is refused as skill_immune (node/server.js:3132). It also cannot be frozen or burned.
    /// </summary>
    ...
    ///     <see cref="Condition.Poisoned" /> (node/server.js:12056).
    /// </summary>
    ...
    ///     (node/server.js:13594). Separately, once 20 seconds pass without it being attacked it gives up pursuit with chance
    ///     1 - rage * 0.99, so a value above about 1.02 never gets bored (:12871).
    /// </summary>
    ...
    ///     same thing, and either one is enough (node/server.js:13016).
    /// </summary>
    ...
    ///     ordinary one (node/server.js:11864, :12404). Set on bosses and event monsters.
    /// </summary>
    ...
    ///     aims its healing ability at that one whenever it is within 120 rather than at itself (node/server.js:12764,
    ///     :12584).
    /// </summary>
    ...
    /// <summary>
    ///     If true, the client leaves this monster out of its monster list (js/html.js:2400). Set on the target dummies and
    ///     one debug monster.
    /// </summary>
    ...
    /// <summary>
    ///     What this monster actually chases at, which is <see cref="ChargeSpeed" /> when the design data names one and a
    ///     multiple of <see cref="AttributedRecordBase.Speed" /> when it does not. Only the explicit half is on the wire: the
