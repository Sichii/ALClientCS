...
/// <summary>
///     Shared positional-element reader: null-safe and forwards <see cref="JsonSerializerOptions" /> so nested
///     converters and number-from-string coercion apply, mirroring Newtonsoft's <c>JToken.ToObject&lt;T&gt;(serializer)</c>.
/// </summary>
...
public sealed class ArrayToTupleConverter<T1, T2, T3> : JsonConverter<ValueTuple<T1?, T2?, T3?>>
{
    ...
    public override void Write(Utf8JsonWriter writer, (T1?, T2?, T3?) value, JsonSerializerOptions options) => throw new NotSupportedException();
}
...
public class ArrayToTupleConverter<T1, T2, T3, T4> : JsonConverter<ValueTuple<T1?, T2?, T3?, T4?>>
{
    ...
    public override (T1?, T2?, T3?, T4?) Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        return arr.Count switch
        {
            ...
            3 => (TupleElement.Read<T1>(arr, 0, options), TupleElement.Read<T2>(arr, 1, options), TupleElement.Read<T3>(arr, 2, options), default),
    ...
    public override void Write(Utf8JsonWriter writer, (T1?, T2?, T3?, T4?) value, JsonSerializerOptions options) => throw new NotSupportedException();
}
...
public sealed class ArrayToTupleConverter<T1, T2, T3, T4, T5> : JsonConverter<ValueTuple<T1?, T2?, T3?, T4?, T5?>>
{
    ...
    public override void Write(Utf8JsonWriter writer, (T1?, T2?, T3?, T4?, T5?) value, JsonSerializerOptions options) => throw new NotSupportedException();
}
...
public class ArrayToTupleConverter<T1, T2, T3, T4, T5, T6> : JsonConverter<ValueTuple<T1?, T2?, T3?, T4?, T5?, T6?>>
{
    ...
    public override void Write(Utf8JsonWriter writer, (T1?, T2?, T3?, T4?, T5?, T6?) value, JsonSerializerOptions options) => throw new NotSupportedException();
}
...
public class ArrayToTupleConverter<T1, T2, T3, T4, T5, T6, T7> : JsonConverter<ValueTuple<T1?, T2?, T3?, T4?, T5?, T6?, T7?>>
{
    ...
    public override (T1?, T2?, T3?, T4?, T5?, T6?, T7?) Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        return arr.Count switch
        {
            ...
            2 => (TupleElement.Read<T1>(arr, 0, options), TupleElement.Read<T2>(arr, 1, options), default, default, default, default, default),
            3 => (TupleElement.Read<T1>(arr, 0, options), TupleElement.Read<T2>(arr, 1, options), TupleElement.Read<T3>(arr, 2, options), default,
            ...
            6 => (TupleElement.Read<T1>(arr, 0, options), TupleElement.Read<T2>(arr, 1, options), TupleElement.Read<T3>(arr, 2, options),
                     TupleElement.Read<T4>(arr, 3, options), TupleElement.Read<T5>(arr, 4, options), TupleElement.Read<T6>(arr, 5, options), default),
...
///     serialized tuple in the model is a positional array (both the single-tuple <c>[JsonConverter]</c> sites and
///     the <c>ItemConverterType</c> list-element sites), so one global factory replaces both. The <c>?</c> on the
///     converter's unconstrained parameters is annotation-only, so the produced converter's runtime target type is
///     the exact declared tuple (e.g. <c>ValueTuple&lt;float, float&gt;</c>), which is what System.Text.Json matches on.
/// </summary>
public sealed class NewtonsoftTupleConverterFactory : JsonConverterFactory
{
