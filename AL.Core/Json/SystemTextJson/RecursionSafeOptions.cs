...
/// <summary>
///     A converter factory that can produce a copy of itself which declines one specific type, so a converter
///     registered VIA this factory can run its inner declared-member fill without the factory re-producing the
///     very converter that is running (nested types still match). Mirrors
///     <c>AttributedObjectConverterFactory.Excluding</c>; needed because a factory cannot be removed by concrete
///     converter type.
/// </summary>
...
/// <summary>
///     Returns a cached copy of a <see cref="JsonSerializerOptions" /> with a specific converter neutralized for
///     the inner deserialize, so a converter that deserializes its OWN handled type (the array-to-object,
///     string-or-object, event/boss and disappear converters) can run the inner declared-member fill without
///     re-entering itself. A converter registered as a concrete instance is dropped by exact type; one registered
///     via an <see cref="IExcludingConverterFactory" /> is instead replaced by a copy that excludes the handled
///     type (removing by type would miss the factory, since the factory's own type never equals the produced
///     converter's type — the defect that made every positional-array type bind to null / string-or-object recurse).
/// </summary>
