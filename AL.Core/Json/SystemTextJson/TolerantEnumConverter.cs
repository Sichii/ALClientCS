...
/// <summary>
///     A per-enum converter that degrades an unrecognized wire value to the enum's zero member rather than
///     throwing, so one unknown value never discards the whole socket frame. The System.Text.Json replacement
///     for the Newtonsoft <c>TolerantStringEnumConverter</c>; produced by
///     <see cref="TolerantStringEnumConverterFactory" />.
/// </summary>
/// <typeparam name="TEnum">
///     An enum type. Parsed with <see cref="EnumHelper" /> so <see cref="System.Runtime.Serialization.EnumMemberAttribute" />
///     aliases are honoured (case-insensitively).
/// </typeparam>
public sealed class TolerantEnumConverter<TEnum> : JsonConverter<TEnum> where TEnum: struct, Enum
{
    ...
    private static TEnum ParseTolerant(string? raw)
    {
        ...
        if (long.TryParse(raw, NumberStyles.Integer, CultureInfo.InvariantCulture, out var underlying))
...
///     <c>[JsonConverter(typeof(TolerantStringEnumConverterFactory))]</c> on an enum type. Nullable enum
///     members are handled by System.Text.Json's built-in nullable wrapper over the produced converter.
/// </summary>
...
///     (<c>Slot</c> -&gt; "mainhand", <c>TradeSlot</c> -&gt; "trade1", <c>BankPack</c> -&gt; "items0").
///     Replaces the Newtonsoft two-arg
///     <c>[JsonConverter(typeof(TolerantStringEnumConverter), typeof(LowerCaseNamingStrategy))]</c>.
/// </summary>
...
/// <summary>
///     Registers <see cref="TolerantEnumConverter{TEnum}" /> for exactly the enums the models mark tolerant with the
///     Newtonsoft <c>[JsonConverter(typeof(TolerantStringEnumConverter))]</c> attribute — reading the
///     <see cref="LowerCaseNamingStrategy" /> second argument to pick the lowercase-emitting variant. An enum without
///     that attribute is left to System.Text.Json's numeric default, matching Newtonsoft, so this must be registered
///     instead of the blanket <see cref="TolerantStringEnumConverterFactory" /> (which would make every enum tolerant).
///     Transitional: reads Newtonsoft attributes so the models need no System.Text.Json enum attributes before Phase 6.
/// </summary>
public sealed class NewtonsoftTolerantEnumConverterFactory : JsonConverterFactory
{
    public override bool CanConvert(Type typeToConvert) => typeToConvert.IsEnum && (TolerantAttribute(typeToConvert) is not null);

    public override JsonConverter CreateConverter(Type typeToConvert, JsonSerializerOptions options)
    {
        var attribute = TolerantAttribute(typeToConvert)!;
        var lowerCase = (attribute.ConverterParameters?.Length > 0) && (attribute.ConverterParameters[0] as Type == typeof(LowerCaseNamingStrategy));

        return (JsonConverter)Activator.CreateInstance(typeof(TolerantEnumConverter<>).MakeGenericType(typeToConvert), new object[] { lowerCase })!;
    }

    private static NJson.JsonConverterAttribute? TolerantAttribute(Type type)
    {
        var attribute = type.GetCustomAttribute<NJson.JsonConverterAttribute>();

        return attribute?.ConverterType == typeof(NsConverters.TolerantStringEnumConverter) ? attribute : null;
    }
}
