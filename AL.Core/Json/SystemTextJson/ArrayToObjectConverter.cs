...
///     <c>ArrayToObjectConverter</c>. Register in the shared options (or reuse <see cref="FromArray" />) rather
///     than as a type-level attribute, so the inner declared-member deserialize can drop this converter and
...
public sealed class ArrayToObjectConverter<T> : JsonConverter<T>
{
    ...
    private static readonly (int Index, MemberInfo Member)[] Indexed =
        typeof(T).GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
                 .Cast<MemberInfo>()
                 .Concat(typeof(T).GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
                 .Select(member => (member, attribute: member.GetCustomAttribute<JsonArrayIndexAttribute>()))
    ...
    private static readonly Dictionary<string, int> IndexByParameterName =
        Indexed.ToDictionary(set => set.Member.Name, set => set.Index, StringComparer.OrdinalIgnoreCase);
    ...
    /// <summary>
    ///     Maps a positional <see cref="JsonArray" /> to <typeparamref name="T" /> by index. Exposed so other
    ///     converters (e.g. the disappear-data converter's spawn orientation) can reuse it without a reader.
    /// </summary>
...
///     members — such a type is positional by design and is therefore always array-shaped on the wire. Registered in
///     the shared options so a single factory covers every <c>ItemConverterType = ArrayToObjectConverter&lt;…&gt;</c>
...
public sealed class ArrayToObjectConverterFactory : JsonConverterFactory, IExcludingConverterFactory
{
    ...
    public override bool CanConvert(Type typeToConvert)
        => (typeToConvert != Excluded)
           ...
           && (typeToConvert.GetProperties(FLAGS).Any(HasIndex) || typeToConvert.GetFields(FLAGS).Any(HasIndex));
