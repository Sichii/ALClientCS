...
/// <summary>
///     Binds a type that Newtonsoft forces to an object contract with <c>[JsonObject]</c> but which System.Text.Json
///     would treat as a collection because it also implements <see cref="IEnumerable" /> — the geometry containers
///     (e.g. <c>GGeometry</c>) that implement <c>IRectangle : IEnumerable&lt;IPoint&gt;</c> for bounding-box
///     convenience yet serialize as a named object. It reads the named object and sets each member the way the
///     binding modifier would (Newtonsoft <c>[JsonProperty]</c> renames, <c>[JsonIgnore]</c>, non-public setters),
///     deserializing each value through the options so nested converters still apply. The positional
///     <c>[JsonArrayIndex]</c> shapes (GDoor/GTile) are handled by <see cref="ArrayToObjectConverter{T}" /> instead.
/// </summary>
public sealed class ForcedObjectConverter<T> : JsonConverter<T> where T: new()
{
    ...
    // that member only - never leaked onto a same-typed sibling.
    private static readonly ConditionalWeakTable<JsonSerializerOptions, ConcurrentDictionary<JsonConverter, JsonSerializerOptions>> MemberOptionsCache = new();
    ...
    //settable public properties and any [JsonProperty] fields — the members the binding modifier would surface for an
    //object; [JsonIgnore] and computed get-only accessors are skipped. Each member's property-level converter is
    //resolved the same way the modifier does, so RIP/AFK (AfkConverter) and Conditions (the tolerant condition-dict
    //converter) bind identically even though these IEnumerable types never reach the modifier's object path.
    private static (string, Type, JsonConverter?, Action<T, object?>)[] BuildMembers()
    {
        ...
        for (var type = typeof(T); (type is not null) && (type != typeof(object)); type = type.BaseType)
        {
            foreach (var property in type.GetProperties(FLAGS))
            {
                ...
                if (property.GetCustomAttribute<NJson.JsonIgnoreAttribute>() is not null)
                    continue;

                if (property.GetSetMethod(nonPublic: true) is not { } setter)
                    continue;

                //Newtonsoft binds a public-setter property by default, but a non-public setter only when the
                //member carries [JsonProperty]. EntityBase.In (public getter, protected setter, no [JsonProperty])
                //therefore stays unbound - the map/in stamp is applied later by ALClient, not at deserialize.
                if (!setter.IsPublic && (property.GetCustomAttribute<NJson.JsonPropertyAttribute>() is null))
                    continue;

                var wireName = property.GetCustomAttribute<NJson.JsonPropertyAttribute>()?.PropertyName ?? property.Name;
                var converter = NewtonsoftAttributeBindingModifier.MapPropertyConverter(property);
                members.Add((wireName, property.PropertyType, converter, (target, value) => setter.Invoke(target, [value])));
            }

            foreach (var field in type.GetFields(FLAGS))
                if (field.GetCustomAttribute<NJson.JsonPropertyAttribute>() is { } attribute)
                    members.Add(
                        (attribute.PropertyName ?? field.Name,
                            field.FieldType,
                            NewtonsoftAttributeBindingModifier.MapPropertyConverter(field),
...
/// <summary>
///     Applies <see cref="ForcedObjectConverter{T}" /> to a type that carries Newtonsoft's <c>[JsonObject]</c> (on
///     itself or an implemented interface such as <c>IRectangle</c>), implements <see cref="IEnumerable" />, exposes a
///     parameterless constructor, and is not a positional <c>[JsonArrayIndex]</c> type. Registered last, so the
///     specific converters (attributed, array-to-object, map-rectangle, …) claim their types first.
/// </summary>
public sealed class ForcedObjectConverterFactory : JsonConverterFactory
{
    ...
    //Newtonsoft [JsonObject] on the type or an implemented interface (e.g. IRectangle) forces an object contract
    private static bool ForcedToObject(Type type)
        => (type.GetCustomAttribute<NJson.JsonObjectAttribute>() is not null)
           || type.GetInterfaces().Any(contract => contract.GetCustomAttribute<NJson.JsonObjectAttribute>() is not null);

    private static bool HasArrayIndex(Type type) => type.GetMembers(FLAGS).Any(member => member.GetCustomAttribute<JsonArrayIndexAttribute>() is not null);
}
