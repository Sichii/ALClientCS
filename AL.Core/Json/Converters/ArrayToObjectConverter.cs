...
/// <summary>
///     Provides conversion logic for objects that are represented as an array of property values. Implements
///     <see cref="JsonConverter" />
/// </summary>
/// <typeparam name="T">
/// </typeparam>
/// <seealso cref="JsonConverter" />
public sealed class ArrayToObjectConverter<T> : JsonConverter
{
    ...
    static ArrayToObjectConverter()
        => PropertyNamesByIndex = typeof(T).GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
                                           ...
                                           .Select(
                                               ...
                                               {
                                                   p,
                                                   p.GetCustomAttribute<JsonArrayIndexAttribute>()
                                                    ?.Index
                                               })
        ...
        JsonSerializer serializer)
    {
        ...
        var obj = new JObject(
            array.Select((jt, i) => PropertyNamesByIndex.TryGetValue(i, out var propName) ? new JProperty(propName, jt) : null)
                 .Where(jp => jp != null));
