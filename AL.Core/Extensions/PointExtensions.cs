...
namespace AL.Core.Extensions
{
    public static class PointExtensions
    {
        ...
        public static Direction DirectionalRelationTo(this IPoint p1, IPoint p2)
        {
            ...
            var degree = 0.0F;
            if (p1.Y.SignificantlyLessThan(p2.Y))
            {
                degree = p2.Y - p1.Y;
                direction = Direction.Up;
            }

            if (p1.X.SignificantlyGreaterThan(p2.X) && degree.SignificantlyLessThan(p1.X - p2.X))
            {
                degree = p1.X - p2.X;
                direction = Direction.Right;
            }

            if (p1.Y.SignificantlyGreaterThan(p2.Y) && degree.SignificantlyLessThan(p1.Y - p2.Y))
            {
                degree = p1.Y - p2.Y;
                direction = Direction.Down;
            }

            if (p1.X.SignificantlyLessThan(p2.X) && degree.SignificantlyLessThan(p2.X - p1.X))
        ...
        public static float Distance(this IPoint p1, IPoint p2)
        {
            ...
            return result.NearlyEquals(0) ? 0 : result;
        }
        
        public static float FastDistance(this IPoint p1, IPoint p2) => (float)(Math.Pow(p2.X - p1.X, 2) + Math.Pow(p2.Y - p1.Y, 2));
