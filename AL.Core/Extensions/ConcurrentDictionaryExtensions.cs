...
public static class ConcurrentDictionaryExtensions
{
    ...
    ///         ToList
    ///     </c>
    ///     on a <see cref="ConcurrentDictionary{TKey,TValue}" /> reads
    ///     <c>
    ///         Count
    ///     </c>
    ///     then
    ...
    ///         CopyTo
    ///     </c>
    ///     as two operations. A writer between them throws <see cref="ArgumentException" />. That is the hazard on the
    ///     <c>
    ...
    ///         Values
    ///     </c>
    ///     and the dictionary's own
    ///     <c>
    ///         ToArray
    ///     </c>
    ///     build their copy under every lock and were never at risk. The enumerator's contract is concurrent-safe.
    ///     <br />
    ...
    ///         TryRemove(kvp)
    ///     </c>
    ///     compares the value with <see cref="EqualityComparer{T}.Default" />, so what it protects depends on
    ///     <typeparamref name="TValue" />. A class with reference equality - a
    ///     <c>
    ///         CooldownInfo
    ///     </c>
    ///     - drops only the instance ticked here, leaving a replacement written this frame alone. A
    ///     <b>
    ///         record
    ///     </b>
    ///     compares by value, so a replacement that happens to be equal is dropped too. That is currently harmless for both
    ///     record callers, and only because neither writes into a live map: an entity's conditions are replaced wholesale on
    ///     update, and a projectile id is unique per shot. Anything that starts writing single entries into one of those maps
    ///     needs a key-and-instance removal instead.
    /// </remarks>
